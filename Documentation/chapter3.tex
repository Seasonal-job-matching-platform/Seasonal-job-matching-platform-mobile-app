...
\texttt{paginatedJobsProvider} & \texttt{AsyncNotifierProvider} & Instantiated on search; resets to page 0 when filters or search terms change. \\ \hline
...
\begin{tikzpicture}[scale=0.85, every node/.style={transform shape}]
    % Define styles
    \tikzstyle{state} = [draw, rectangle, fill=orange!10, text width=3cm, minimum height=1cm, align=center, rounded corners]
    
    % Nodes
    \node[state] (splash) at (0, 6) {SplashWrapper\\(Session Check)};
    \node[state] (login) at (-3, 4) {LoginScreen};
    \node[state] (signup) at (-3, 2.5) {SignupScreen};
    \node[state] (layout) at (3, 4) {LayoutScreen\\(Main Shell)};
    
    \node[state] (home) at (6, 6) {Home Tab\\(Carousel)};
    \node[state] (search) at (6, 4.5) {Search Tab\\(Filters)};
    \node[state] (apps) at (6, 3) {Applications Tab\\(Timeline)};
    \node[state] (profile) at (6, 1.5) {Profile Tab\\(Resume)};
    
    % Flow arrows
    \draw[->, thick] (splash) -- node[midway, above left] {No Session} (login);
    \draw[->, thick] (splash) -- node[midway, above right] {Valid Session} (layout);
...
\end{tikzpicture}
\caption{Application Navigation and Screen Flow}
