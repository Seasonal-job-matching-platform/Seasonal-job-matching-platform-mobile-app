...
\textbf{FR-PROF-4} & The system shall support user selection of a preferred currency in the profile settings, persisting it on the backend database and propagating selections to update and format all salary listings dynamically. \\ \hline
\textbf{FR-PROF-5} & The system shall provide a feedback and bug reporting form in settings, permitting users to submit issues either anonymously or automatically including their logged-in email. \\ \hline
...
\textbf{FR-UPD-1}  & The system shall query the GitHub Releases API on startup to check for newer application versions. \\ \hline
\textbf{FR-UPD-2}  & The system shall classify found updates as mandatory if the major version is incremented or if the release contains the `[MANDATORY]` tag; otherwise, classify as optional. \\ \hline
\textbf{FR-UPD-3}  & The system shall intercept navigation and display a non-dismissible full-screen overlay blocking access to the app if a mandatory update is found. \\ \hline
\textbf{FR-UPD-4}  & The system shall prompt optional updates using a bottom sheet, caching a 24-hour snooze state in secure storage upon dismissal and rendering visual update badges on profile settings. \\ \hline
\end{tabular}
\end{table}
...
\begin{tikzpicture}[scale=0.9, every node/.style={transform shape}]
    % Actors
    \node[draw, minimum width=1cm, minimum height=1.5cm] (applicant) at (0, 3) {Applicant};
    \node[draw, minimum width=1cm, minimum height=1.5cm, align=center] (api) at (10, 4.5) {Backend\\API};
    \node[draw, minimum width=1cm, minimum height=1.5cm, align=center] (fcm) at (10, 1.5) {Firebase\\FCM};
    \node[draw, minimum width=1cm, minimum height=1.5cm, align=center] (github) at (10, -1.5) {GitHub\\API};

    % Boundaries
    \draw (2,-3.5) rectangle (8,9.5);
    \node at (5, 9.1) {\textbf{Mobile Client Boundary}};

    % Use Cases
    \node[draw, ellipse, align=center] (uc1) at (5, 8.1) {Register / Login};
    \node[draw, ellipse, align=center] (uc2) at (5, 6.8) {Search \& Filter\\Job Listings};
    \node[draw, ellipse, align=center] (uc3) at (5, 5.4) {View Job Details};
    \node[draw, ellipse, align=center] (uc4) at (5, 4.0) {Apply for Job};
    \node[draw, ellipse, align=center] (uc5) at (5, 2.6) {Manage Resume \&\\Fields of Interest};
    \node[draw, ellipse, align=center] (uc6) at (5, 1.2) {Track Application\\Timeline};
    \node[draw, ellipse, align=center] (uc7) at (5, -0.2) {Receive Notifications};
    \node[draw, ellipse, align=center] (uc8) at (5, -1.4) {Select Currency \&\\Submit Feedback};
    \node[draw, ellipse, align=center] (uc9) at (5, -2.6) {Check \& Apply\\OTA Updates};
    ...
    % Connections (Backend/FCM/GitHub)
    \draw[->] (uc1) -- (api);
    \draw[->] (uc2) -- (api);
    \draw[->] (uc3) -- (api);
    \draw[->] (uc4) -- (api);
...
\subsection{User Story 4: Preferred Currency Selection}
\textbf{As an} Applicant,\\
\textbf{I want to} select my preferred currency in the profile settings,\\
\textbf{So that} I can view all job salaries formatted automatically in my local denomination.
\begin{itemize}
    \item \textbf{Acceptance Criteria 1:} The settings page must render a currency selection dropdown showing supported codes (EGP, USD, EUR, JOD, SAR).
    \item \textbf{Acceptance Criteria 2:} Selecting a currency must trigger a PATCH call to the profile API and invalidate the job feed cached providers.
    \item \textbf{Acceptance Criteria 3:} Salary values on all screens (cards, details, recommendations, favorites) must format reactively via localized currency symbols.
\end{itemize}

\subsection{User Story 5: Automated OTA Update Verification}
\textbf{As an} Applicant,\\
\textbf{I want the system to} automatically detect newer software releases on startup and guide me to update,\\
\textbf{So that} I remain on a secure, fully supported version of the mobile client.
\begin{itemize}
    \item \textbf{Acceptance Criteria 1:} If a major version mismatch is found on startup, navigation must be blocked by a non-dismissible full-screen overlay showing release notes and a direct download button.
    \item \textbf{Acceptance Criteria 2:} Optional version upgrades must prompt a bottom sheet, allowing the user to select ``Later''.
    \item \textbf{Acceptance Criteria 3:} Dismissing an optional update must snooze the startup sheet modal for exactly 24 hours, displaying a visual badge dot next to the profile menu instead.
\end{itemize}

\subsection{User Story 6: Feedback Submission with Anonymous Toggling}
\textbf{As an} Applicant,\\
\textbf{I want to} submit bug reports and feedback from the profile screen, with the option to send it anonymously,\\
\textbf{So that} I can report software issues while maintaining privacy if desired.
\begin{itemize}
    \item \textbf{Acceptance Criteria 1:} The feedback form dialog must require a title and a description.
    \item \textbf{Acceptance Criteria 2:} An ``Anonymous'' toggle switch must exclude the user's logged-in email from the network payload when checked.
    \item \textbf{Acceptance Criteria 3:} Submitting feedback must display a loading spinner, followed by a success dialog toast.
\end{itemize}
...
\textbf{Reliability} & Update Fail-Silence & GitHub API connection errors or rate limit ceilings shall bypass updates silently within $500\text{ms}$ to allow uninterrupted startup. \\ \hline
...
\textbf{Usability} & Currency Format Latency & Salary formatting updates across all visible list widgets must complete within $50\text{ms}$ of changing the preferred currency. \\ \hline
\end{tabular}
\end{table}
